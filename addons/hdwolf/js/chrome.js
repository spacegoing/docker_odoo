odoo.define('point_of_sale.chrome', function(require) {
        ...
        start: function() {
            ...
            this.$('.button.import_orders input').on('change', function(event) {
                ...
                if (file) {
                    ...
                    reader.onload = function(event) {
                        ...
                        self.gui.show_popup('orderimport', {
                            report: report
                        });
                    };
            ...
            _.each(this.events, function(name) {
                self.pos.proxy.add_notification(name, function() {
                    self.$('.event.' + name).stop().clearQueue().css({
                        'background-color': '#6CD11D'
                    });
                    self.$('.event.' + name).animate({
    ...
    var Chrome = PosBaseWidget.extend({
        ...
        init: function() {
            ...
            this.pos = new models.PosModel(this.session, {
                chrome: this
            });
            ...
            function wolf_go_deduct(barcode) {
                if (barcode) {
                    // barcode = 'suki bakery 进口酸奶粉原味230g新西兰原装';

                    // Select all prod orderlines
                    // let items = $("li[class~=orderline]:contains('"+barcode+"')");
                    // items.last().attr('class','orderline selected');
                ...
                function waitforbarcode_go_deduct() {
                    return self.pos.proxy.connection_wolf.rpc('/go_deduct', {}, {
                        ...
                        .then(function(barcode) {
                                wolf_go_deduct(barcode);
                                waitforbarcode_go_deduct();
                            },
                            function() {
                                setTimeout(waitforbarcode_go_deduct, 5000);
                            });
                }
                waitforbarcode_go_deduct();
        ...
        loading_progress: function(fac) {
            ...
            this.$('.loader .progress').removeClass('oe_hidden').css({
        ...
        loading_show: function() {
            this.$('.loader').removeClass('oe_hidden').animate({
                opacity: 1
            }, 150, 'swing');
        },
        widgets: [{
