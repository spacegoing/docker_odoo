...
var Chrome = PosBaseWidget.extend({
    ...
    init: function() { 
        ...
        function wolf_go(del_prod_name){
              if(del_prod_name){
                  // del_prod_name = 'suki bakery 进口酸奶粉原味230g新西兰原装';

                  // let items = $("li[class~=orderline]:contains('"+del_prod_name+"')");
                  ...
                  orderlines.models.forEach(function(item,index,models){
                      if(item.product.display_name===del_prod_name){
        ...
        this.pos.ready.done(function(){
            ...
            self.pos.push_order();
            function waitforbarcode_go(){
                return self.pos.proxy.connection_wolf.rpc('/go',{},{timeout:7500})
                    .then(function(del_prod_name){
                        // if(!self.remote_scanning){
                        //     self.remote_active = 0;
                        //     return;
                        // }
                        wolf_go(del_prod_name);
                        waitforbarcode_go();
                    },
                          function(){
                              // if(!self.remote_scanning){
                              ...
                              //     return;
                              // }
                              setTimeout(waitforbarcode_go,5000);
                          });
            }
            waitforbarcode_go();
